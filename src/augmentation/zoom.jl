...
"""
    zoom!(out::Array{Float32,3}, img::Array{Float32,3}, scale::Float32)
...
# Arguments
- `out::Array{Float32,3}` : pre-allocated output buffer of the same size as `img`.
- `img::Array{Float32,3}` : input image to zoom.
...
"""
function zoom!(out::Array{Float32,3}, img::Array{Float32,3}, scale::Float32)
    H, W, C = size(img)
